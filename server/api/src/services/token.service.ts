...
// שרענון שהצליח בשרת אבל איבד את התשובה בלקוח לא ינתק את המשתמש.
// ראה RefreshToken.model.ts + findByPreviousToken.
//
// 60 שניות היה קצר מדי בפועל: כל reload/סגירת אפליקציה מבטל את הבקשה
// שממתינה לתשובה מ-/auth/refresh בצד הלקוח, אבל השרת כבר השלים את ה-rotation
// לפני שהחיבור נותק - הלקוח נשאר עם refresh token ישן שרק "replay" בתוך
// חלון החסד מציל אותו. כמה F5/סגירות ברצף (או תגובה איטית אחת - Render
// cold start יכול לקחת 30-50 שניות) עלולים לחצות 60 שניות מצטבר לפני
// שאיזושהי תשובה בכלל נשמרה בהצלחה בלקוח - ואז הלקוח נשאר "יתום" לצמיתות
// עם טוקן שלא current ולא בתוך חלון חסד יותר, מקבל 401 אמיתי ומתנתק בפועל.
// זו הייתה תופעה נצפית: "אחרי כמה רענונים/סגירות המשתמש מתנתק".
