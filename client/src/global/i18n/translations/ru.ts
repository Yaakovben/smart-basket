...
export const ru: Translations = {
  ...
  feedbackPlaceholder: 'Что можно улучшить? Чего бы вам хотелось?',
  feedbackSend: 'Отправить',
  feedbackMaybeLater: 'Может позже',
  feedbackThanks: 'Спасибо за отзыв!',
  feedbackRatingPrompt: 'Оцените свой опыт',
  feedbackRatingAria: 'Оценка пользовательского опыта',
  feedbackRating1: 'Плохо',
  feedbackRating2: 'Сойдёт',
  feedbackRating3: 'Хорошо',
  feedbackRating4: 'Очень хорошо',
  feedbackRating5: 'Отлично!',
  // About
  ...
  weekdayFridayPrep: 'Готовитесь к субботе? 🕯️',
  weekdaySaturdayNight: 'Новая неделя 💪',
  weekdaySundayMorning: 'Начинается новая неделя 🚀',
  search: 'Поиск...',
  ...
  qrPasswordMissing: 'Код распознан, но пароль отсутствует или неверен — введите пароль вручную.',
  qrNotGroupCode: 'Это не QR-код для вступления в группу — отсканируйте QR, полученный от организатора.',
  recentlyAdded: 'Недавно добавленные',
  betaBadgeLabel: 'Бета-версия',
  ...
  photoSyncFailed: 'Фото сохранено на устройстве, но не загружено в облако.',
  photoRetryUpload: 'Повторить загрузку',
  photoSavedToCloud: 'Фото сохранено',
  photoDropHint: 'Перетащите сюда, чтобы добавить фото',
  ...
  purchasesNoPricesTitle: 'Пока нет оценки расходов',
  purchasesNoPricesDesc: 'Вы отметили {count} товаров купленными в этом месяце, но ни один из них пока не сопоставлен с ценой — поэтому нечего суммировать. Оценка появится, как только для этих товаров найдутся цены.',
  ...
  aiOfflineHint: 'Нет подключения к интернету - попробуйте снова, когда связь восстановится',
  ...
  priceHelpTitle: 'Как это работает?',
  priceHelpIntro: 'Несколько коротких советов для более точного сравнения цен.',
  priceHelpBranchTitle: 'Выбор филиала',
  priceHelpBranchDesc: 'На каждой карточке сети есть кнопка "Сменить филиал" - можно найти и выбрать конкретный филиал вместо ближайшего по умолчанию. Цены обновятся под выбранный филиал.',
  priceHelpMatchTitle: 'Исправление сопоставления товара',
  priceHelpMatchDesc: 'Если товар определён неверно (или не найден), нажмите "Не тот товар?" рядом со строкой и найдите нужный. Можно также отметить "Ничего из этого", если товара там нет. Выбор запомнится.',
  priceHelpVerifiedTitle: 'Подтверждённая цена и оценка',
  priceHelpVerifiedDesc: 'Зелёная отметка рядом с ценой означает, что она подтверждена в самом филиале. Без неё - это оценка по обычной цене сети, в магазине может немного отличаться.',
  ...
  priceMayDifferAtBranch: 'Эта цена - оценка по всей сети, не проверена в ближайшем к вам магазине, и может отличаться на месте',
  cheaperAtBranch: 'Дешевле (₪{price}) в магазине {branch}',
  ...
  changeBranch: 'Сменить филиал',
  chooseBranchTitle: 'Выбор филиала {chain}',
  useNearestBranch: 'Ближайший ко мне',
  branchChosenByYou: 'Выбран вами',
  branchHasNoPriceData: 'Нет данных о ценах',
  branchPricesVerified: '{n} из {m} цен подтверждены в филиале',
  staleChainData: 'Данные {days} дн. назад',
  lowConfidenceMatch: 'Проверьте, тот ли это товар',
  splitSavingsHint: 'Разделив покупки между сетями, сэкономите ещё {amount}',
  matchPickerTitle: 'Какой товар вы имели в виду?',
  matchPickerSubtitle: 'Выбор сохранится для всех сетей и списков',
  matchPickerSearchPlaceholder: 'Поиск товара',
  matchPickerNoResults: 'Ничего не найдено. Попробуйте другое слово',
  matchPickerInChains: 'в {count} сетях',
  matchPickerFrom: 'от',
  matchPickerNoneOfThese: 'Ничего из этого',
  matchPickerReset: 'Вернуть автоподбор',
  matchPickerSaveFailed: 'Не удалось сохранить',
  matchFixAction: 'Искать вручную',
  matchNotRightAction: 'Не тот товар?',
  ...
  noPendingItemsInList: 'Нет некупленных товаров в списке',
  ...
  noMatchInListHint: 'Не удалось определить ни одного товара из списка в базе данных.',
  ...
  selectedOfCount: 'из {total}',
  itemHasNote: 'У этого товара есть заметка',
  longPressMultiSelectHint: 'Совет: удерживайте товар, чтобы выбрать несколько',
  dismissHint: 'Скрыть подсказку',
  aiAndInsights: 'AI и аналитика',
  listAnalysisTitle: 'Анализ списка',
  analyzingList: 'Анализирую список...',
  noAnalysisThisTime: 'На этот раз текстовый анализ не получен - попробуйте позже. Оценка цены ниже всё ещё точна.',
  estimatedBasketCost: 'Ориентировочная стоимость корзины',
  itemsIdentifiedOfTotal: '{matched} из {total} товаров определено',
  priceComparisonShort: 'Сравнение цен',
  tellMeMoreAboutList: 'Расскажи подробнее о списке "{name}" и как его улучшить',
  ...
  savedListNameExample: 'Например: еженедельные покупки',
  chooseIcon: 'Выберите значок',
  savedListSaved: 'Готовый список сохранён',
  savedListApplied: 'товаров добавлено',
  savedListAllPresent: 'Все товары уже в списке',
  manageSavedLists: 'Управление готовыми списками',
  ...
  savedListsChooserApplyDesc: 'Добавьте товары из готового списка, который уже есть',
  savedListsChooserCreateTitle: 'Создать новый готовый список',
  savedListsChooserCreateDesc: 'Сохраните товары текущего списка как новый готовый список',
  savedListsChooserCreateDisabledDesc: 'Добавьте товары в список, чтобы сохранить его',
  ...
  cacheNoticeTitle: 'Мы обновили приложение',
  cacheNoticeBody: 'После недавних обновлений приложение может зависнуть на "подключение к серверу", или действия могут не срабатывать. Если это произошло — нажмите здесь для быстрого, разового сброса (вы не выйдете из аккаунта).',
  cacheNoticeCta: 'Быстрый сброс',
  cacheNoticeDismiss: 'У меня всё в порядке, спасибо',
  ...
  tipEyebrow: 'А вы знали?',
  tipGotIt: 'Отлично, понятно!',
  tipNext: 'Далее',
  tipDotAria: 'Совет {num}',
  tipDontShowAgain: 'Больше не показывать',
  tipScanListTitle: 'Сканируйте список с фото',
  tipScanListBody: 'Есть список от руки? Сфотографируйте его через меню «Ещё» → «Сканировать список», и товары добавятся автоматически.',
  tipProductPhotoTitle: 'Фото товара',
  tipProductPhotoBody: 'При добавлении или изменении товара можно прикрепить фото — чтобы тот, кто покупает, знал точный бренд и упаковку.',
  tipProductNoteTitle: 'Заметка к товару',
  tipProductNoteBody: 'При добавлении или изменении товара можно написать короткую заметку — любимый бренд, размер, или любую важную деталь.',
  tipProductReorderTitle: 'Расставьте товары',
  tipProductReorderBody: 'Кнопка «Сортировать товары» позволяет перетащить каждый товар туда, где вам удобно — например, в порядке обхода магазина.',
  tipListReorderTitle: 'Расставьте списки',
  tipListReorderBody: 'На главном экране кнопка «Сортировать списки» позволяет перетащить карточку списка на новое место и расставить всё по-своему.',
  tipAiAssistantTitle: 'Умный помощник по покупкам',
  tipAiAssistantBody: 'Кнопка AI ✨ позволяет спросить о магазинах и ценах и получить рекомендации на основе ваших реальных расходов и привычек.',
  tipPriceComparisonTitle: 'Сравнение цен между сетями',
  tipPriceComparisonBody: 'Вкладка «Сравнение цен» показывает, где ваш список дешевле всего, товар за товаром, среди всех крупных сетей.',
  tipShareWhatsappTitle: 'Быстрая отправка в WhatsApp',
  tipShareWhatsappBody: 'Меню «Поделиться» позволяет отправить список аккуратным сообщением в WhatsApp одним нажатием.',
  tipSharePdfTitle: 'Список в формате PDF',
  tipSharePdfBody: 'В меню «Поделиться» есть и «Поделиться как PDF» — список экспортируется аккуратным файлом для печати или сохранения.',
  tipSavedListsTitle: 'Сохранённые списки',
  ...
  tipDragFabTitle: 'Кнопку добавления можно перетаскивать',
  ...
  subscriptionCancelTitle: 'Отмена автопродления',
  ...
  upgradeBestValueBadge: 'Выгоднее всего',
  perMonthShort: 'месяц',
};
