                  ...
                  <Box sx={{ flex: 1 }}>
                    ...
                    {pushError === 'PERMISSION_DENIED' ? (
                      ...
                      <Typography sx={{ fontSize: 12, color: 'error.main', mt: 0.5 }}>
                        {pushError === 'NOT_CONFIGURED' ? t('pushErrorNotConfigured')
                          : pushError === 'SAVE_FAILED' ? t('pushErrorSaveFailed')
                          : pushError.startsWith('SUBSCRIBE_FAILED:') ? `* ${pushError.split(':').slice(1).join(':')}`
                          : t('pushErrorUnknown')}
                      </Typography>
                    ) : null}
                  </Box>
