    ...
    <div style={S.screen}>
      ...
      <div style={S.bottomNav}>
        <div style={{ ...S.navItem, background: '#F0FDFA' }}>
          ...
          <span style={{ fontSize: '11px', color: '#14B8A6', fontWeight: '600' }}>ביתPR</span>
        </div>
...
function LoginScreen({ onLogin }: LoginScreenProps) {
  ...
  return (
    <div style={{
      ...
      {/* Card Container - Flex column with max height for internal scroll */}
      <div style={{
        ...
        {/* Fixed Header - Logo (never scrolls) */}
        ...
        {/* Fixed Footer - Error & Submit (never scrolls) */}
  ...
  const ListScreenWrapper = () => {
    ...
    return (
      <ListScreen
        ...
        user={user!}
        onBack={() => navigate('/')}
        onUpdateList={u => { setLists(lists.map(l => l.id === u.id ? u : l)); }}
        onLeaveList={id => { handleLeaveList(id); navigate('/'); }}
        onDeleteList={id => { setLists(lists.filter(l => l.id !== id)); showToast('נמחק!'); navigate('/'); }}
        showToast={showToast}
      />
    );
  };
        ...
        <Route path="/" element={
          <ProtectedRoute>
            <HomeScreen
              ...
              onCreateList={l => { setLists([...lists, l]); showToast('נוצר!'); }}
