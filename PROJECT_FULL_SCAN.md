...
> מסמך זה נוצר ומתעדכן ע"י סריקה ישירה של הקוד בפועל (routes, models, package.json, מבנה תיקיות) — עודכן לאחרונה 2026-08-17. מיועד להזנה לכלי AI (כמו GPT) ליצירת PDF/מצגת/מסמך נוסף.
...
| Email | Brevo HTTP API (fetch ישיר, בלי SDK) | - | שליחת מייל בלי לנהל דומיין SMTP משלנו — כולל טיפול ב-List-Unsubscribe headers כדי לא להיתפס כספאם |
...
### Email — `/api/email` (אדמין בלבד)
- `GET /status` — האם שליחת מייל מוגדרת (`BREVO_API_KEY` קיים)
...
| **איתור סניף קרוב** | Geolocation בדפדפן להשוואת מרחק לסניפים |
| **תובנות (Insights)** | הוצאות מצטברות, מוצרים נפוצים, סטטיסטיקות קבוצתיות — aggregation pipelines |
| **עוזר AI** | צ'אט שמנתח את הרשימה/ההוצאות ועונה על שאלות; Groq (Llama 3.3 70B, ראשי — מהיר, LPU ייעודי) עם נפילה אוטומטית ל-NVIDIA NIM אם Groq לא זמין/חרג ממכסה, כדי שהצ'אט תמיד יענה |
...
| **שליחת מייל מהאדמין** | Brevo HTTP API — שידור לכולם / רק למי שלא הפעיל push / למשתמש ספציפי, מנוהל מתוך אותו מודל כמו Push broadcast (מתג Push/Email), כולל דוח per-user (נשלח/נכשל/דולג) |
| **Geocoding עם גיבוי** | Nominatim (חינמי) כברירת מחדל לאיתור קואורדינטות סניף; נפילה ל-LocationIQ אם Nominatim נכשל על כתובת בעברית |
...
*מסמך זה נוצר ומתעדכן ע"י קריאה ישירה של קובצי הקוד (models, routes, package.json) — עודכן לאחרונה 2026-08-17 — לא מבוסס על הנחות או תיעוד ישן.*
