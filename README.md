...
| `branch_prices` | שורה לכל (סניף, ברקוד) **רק** כשמחיר הסניף שונה מהנפוץ, או כשהמוצר נמכר בפחות מ-90% מהסניפים |
...
**תקציב שורות (רשת ביטחון):** שורת חריגה עולה כ-130 בתים (נתונים ואינדקסים). לכל רשת מותרות עד **150 אלף** שורות חריגה (`MAX_EXCEPTION_ROWS_PER_CHAIN`, `services/branchPricing.ts`). רשת שחורגת, כלומר מחירים שמשתנים הרבה בין סניפים, **לא נשמרת ברמת סניף בכלל**: מוחקים את שורותיה ואת הכיסוי שלה, והיא נשארת בהשוואה ברמת רשת מסומנת "לא מאומת" (ההתראה `exceed budget` בלוג הסנכרון). זו החלטה מכוונת: נתונים חלקיים היו גרועים יותר, כי שורה חסרה למוצר בכיסוי גבוה מוסקת כמחיר הנפוץ.
