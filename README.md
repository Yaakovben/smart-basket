...
Price:                מחיר לכל (ברקוד, רשת): מינימום, מחיר נפוץ (modalPrice), כיסוי סניפים (storeCoverage)
BranchPrice:          מחיר בסניף - חריגות בלבד (שונה מהנפוץ, או מוצר בכיסוי נמוך), עם syncedAt
ChainPriceCoverage:   אילו סניפים הופיעו בפיד המחירים האחרון של הרשת
PriceMatchOverride:   תיקון התאמת מוצר של משתמש (שם מנורמל -> ברקוד, או "אין התאמה")
```
פירוט ההיגיון והאחסון: [השוואת מחירים](#השוואת-מחירים).
...
|--------|------|--------|--------|
| GET | `/` | השוואת מחירי רשימה בין רשתות. פרמטרים: `listId`, `lat`, `lng`, `branches=chain:store,...` (סניפים שנבחרו ידנית) | משתמש |
| GET | `/barcode/:barcode` | חיפוש מוצר לפי ברקוד | משתמש |
| GET | `/branches-nearby` | סניפים קרובים למיקום | משתמש |
| GET | `/chain-branches/:chainId` | סניפי רשת לבורר סניף (עם מרחק, ומי שיש לו נתוני מחיר ראשון) | משתמש |
| GET | `/search?q=` | חיפוש מוצר לבחירה ידנית, מאוחד לפי ברקוד | משתמש |
| PUT | `/overrides` | שמירת תיקון התאמה: `{ productName, barcode }` או `{ productName, excluded: true }` | משתמש |
| DELETE | `/overrides?productName=` | ביטול תיקון, חזרה להתאמה אוטומטית | משתמש |
...
השוואת מחירי הסל של המשתמש בין 14 רשתות, על נתוני חוק שקיפות המחירים. הקוד ב-`server/api/src/features/priceComparison` (שרת) וב-`client/src/features/priceComparison` (קליינט), והמסך בטאב "מחירים" של התובנות.

### איך הנתונים נשמרים (ולמה ככה)

אשכול Atlas חינמי מוגבל ל-512MB, ורשת אחת מפרסמת מאות אלפי שורות (סניף × מוצר). מדידה על נתוני אמת הראתה ש-89%-95% מהמחירים בסניף זהים למחיר הנפוץ ברשת, ולכן שומרים חריגות בלבד:

| אוסף | מה נשמר |
|------|---------|
| `prices` | שורה לכל (ברקוד, רשת): המחיר הזול ביותר, **המחיר הנפוץ** (`modalPrice`), **כיסוי הסניפים** (`storeCoverage`, 0-1), ועוד שדות מהפורטל |
| `branch_prices` | שורה לכל (סניף, ברקוד) כשמחיר הסניף שונה מהנפוץ, ובנוסף (בתקציב) כשהמוצר נמכר בפחות מ-90% מהסניפים |
| `chain_price_coverage` | מסמך לרשת: אילו סניפים הופיעו בפיד האחרון |
| `branches` | סניפים, כתובות וקואורדינטות |
| `price_match_overrides` | תיקוני התאמה של משתמשים (עד 300 למשתמש) |

**קריאת מחיר בסניף** (`services/branchPricing.ts`, `resolveBranchPrice`):
1. יש שורה ב-`branch_prices` → המחיר המפורש, מאומת.
2. אין שורה, הסניף מופיע ב-`chain_price_coverage` והמוצר בכיסוי של 90% ומעלה → המחיר הנפוץ, מאומת ומסומן כמוסק (`branchPriceInferred`).
3. אחרת → המחיר הנפוץ ברשת כהערכה, לא מאומת. בלי מחיר נפוץ (נתונים ישנים) חוזרים למחיר הזול ברשת.

מזהי סניף מנורמלים בכל השכבות (בלי אפסים מובילים, `services/storeId.ts`): "012" ו-"12" הם אותו סניף.

**תקציב שורות (רשת ביטחון):** שורת חריגה עולה כ-130 בתים (נתונים ואינדקסים), ולכל רשת מותרות עד **175 אלף** שורות (`MAX_EXCEPTION_ROWS_PER_CHAIN`, `services/branchPricing.ts`). שתי סיבות לשמור שורה, והן מטופלות אחרת:
- **מחיר שונה מהנפוץ (חובה):** בלי השורה ההיסק מהמחיר הנפוץ היה שגוי. תמיד נשמרות.
- **מוצר בכיסוי נמוך במחיר הנפוץ (רצוי):** מידע על זמינות בסניף. נשמרות רק אם הסך (חובה ורצוי) נכנס לתקציב. אחרת המוצר נשאר "מחיר נפוץ, לא מאומת", וזה נכון כי לא ידוע אם הסניף מוכר אותו. בפועל זה הגורם העיקרי לנפח: ברשתות שכל סניף מחזיק תת-קטלוג, 90% מהשורות הן מהסוג הזה.
- **רשת שגם שורות החובה שלה חורגות** (מחירים שמשתנים הרבה בין סניפים) לא נשמרת ברמת סניף בכלל: שורותיה וכיסויה נמחקים, והיא נשארת בהשוואה ברמת רשת מסומנת "לא מאומת" (ההתראה `exceed budget` בלוג הסנכרון). הלוג מציג `priceDiffers` מול `lowCoverage` לכיוונון.

### סנכרון: מתי ואיך

- **תזמון:** `node-cron`, פעם ביום ב-**04:00 שעון ישראל** (`0 4 * * *`), בתוך שרת ה-API בכל סביבה שאינה development (`jobs/priceSync.job.ts`).
- **חשוב במסלול Free של Render:** ה-cron רץ רק כשהשרת ער. ה-keep-alive שמתואר ב"תשתית פרודקשן" הוא תנאי לכך שהסנכרון יתבצע.
- **בעליית שרת:** אם הנתונים ישנים מ-72 שעות, סנכרון רץ 30 דקות אחרי העלייה.
- **ידני:** `POST /api/price-comparison/refresh` (אדמין), או מהמחשב: `cd server/api && npm run build && node dist/features/priceComparison/scripts/refresh-prices.js`. הסקריפט **כותב** ל-DB שב-`.env`. רשת גדולה לוקחת 3-11 דקות, וכל הרשתות כשעה עד שעתיים.
- **סדר הריצה היומית:** מחירי כל הרשתות ברצף (3 שניות בין רשתות, כשל ברשת אחת לא עוצר את השאר) ← טעינת סניפים מוכרים ← סניפים מ-OpenStreetMap ← מחיקת מחירים ישנים מ-14 יום (`prices`, בקבוצות) ← גיאוקודינג של עד 50 סניפים.
- **מה קורה לכל רשת** (`services/priceSync.service.ts`): הורדת קבצי המחירים ← אגרגציה לפי ברקוד (מחיר מינימלי, נפוץ, כיסוי) ← שמירה ב-`prices` ← סנכרון סניפים ← שמירת חריגות ב-`branch_prices` עם חותמת סנכרון ← מחיקת שורות מסנכרון קודם ורישום `chain_price_coverage`. המחיקה והרישום מתבצעים רק אחרי שכל הכתיבות הצליחו ורק לפיד שיש בו סניפים, כך שפיד ריק לא מוחק נתונים.

### איך ההשוואה מחושבת

1. **התאמת מוצר** (`services/productMatcher.ts`): נרמול שם, התאמת מילים ושורשים, ציון מינימלי 0.55, עם קנסות על קטגוריות צד (למשל "שוקולד חלב" מול "חלב").
2. **כיסוי:** שאילתת מועמדים משותפת, ואם היא התמלאה עד הגבול, השלמה ייעודית לכל רשת עם מעט מועמדים.
3. **אותו מוצר בדיוק:** הברקוד ש-2 רשתות ומעלה התאימו אליו נבחר, ורשתות שמחזיקות אותו מועברות אליו (`services/chainComparison.ts`).
4. **תיקוני משתמש** גוברים על הכול (`services/matchOverrides.ts`). רשת שלא מוכרת את המוצר שנבחר נשארת בלי התאמה ולא מנחשת.
5. **דירוג "הכי זול"** (`services/cheapestRanking.ts`): רק על תת-הסל המשותף. רשת שכיסתה פחות מ-60% ממה שהרשת המכסה ביותר כיסתה לא נכנסת לדירוג. כשלפחות 60% מהרשתות מאומתות בסניף, משווים מחירים מאומתים בלבד.
6. **מטמון:** 15 דקות לכל משתמש, מתאפס בשינוי רשימה, מוצר או תיקון התאמה.

### מה המשתמש רואה ועושה

- כרטיס לכל רשת עם סכום, כמה מוצרים זוהו, מרחק לסניף, חיווי "✓ מחירי הסניף" ואזהרה כשנתוני הרשת ישנים מ-3 ימים.
- **בחירת סניף ידנית** לכל רשת ("החלף סניף"): נשמרת במכשיר ונשלחת בפרמטר `branches`.
- **תיקון התאמה** ("לא המוצר הנכון?"): חיפוש מוצר ובחירה, "אף אחד מאלה", או חזרה להתאמה אוטומטית. חל על כל הרשתות והרשימות.
- **פיצול קניות:** הצעה לחיסכון נוסף כשקונים כל מוצר ברשת הזולה שלו (מוצג מ-5 ש"ח).
- ברירת מחדל: מיון לפי מחיר, והסימון הירוק על הזולה באמת.

### בדיקה ותפעול

```bash
cd server/api
npm test                  # בונה ומריץ את הבדיקות (node:test, בלי תלויות נוספות)
npm run verify-store-ids  # קריאה בלבד: התאמת סניפי הפיד לקובץ הסניפים ונפח החריגות
```

- **מזהי סניף:** אם `verify-store-ids` מראה מעט "תואמים", מחירי הסניף נופלים להערכה ברשת. יש להשוות את דוגמאות המזהים שהוא מדפיס.
- **נפח:** התקציב לרשת מחזיק את `branch_prices` על עד כ-320MB במקרה הקיצוני (14 רשתות × 175 אלף שורות), ובפועל הרבה פחות, ועוד כ-90MB ל-`prices`. בדיקה: `db.stats()` והשוואה ל-512MB. רשת ב"exceed budget" בלוג אינה תקלה, אלא רשת שחוזרת להשוואה ברמת רשת.
- **רשת שנכשלת בסנכרון** (למשל `ECONNRESET` מהפורטל): הרשתות האחרות ממשיכות. הנתונים הקודמים שלה נשארים, והקליינט מזהיר כשהם ישנים מ-3 ימים.
- **CI:** ה-workflow מריץ `npm test` בשרת.
- **הרשאות:** משתמש חינמי מוגבל במספר השוואות ליום (`PLAN_LIMITS`).
...
cd server/api && npm test          # בדיקות השוואת המחירים (node:test)
```
