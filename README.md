...
Price:                מחיר לכל (ברקוד, רשת): מינימום, מחיר נפוץ (modalPrice), כיסוי סניפים (storeCoverage), חריגות מחיר לסניף (storePrices)
...
רשת אחת מפרסמת מאות אלפי שורות (סניף × מוצר), ומדידה על נתוני אמת הראתה שרוב המחירים בסניף זהים למחיר הנפוץ ברשת. לכן שומרים **חריגות בלבד, בתוך מסמך ה-Price**:

| אוסף | מה נשמר |
|------|---------|
| `prices` | מסמך לכל (ברקוד, רשת): המחיר הזול ביותר, **המחיר הנפוץ** (`modalPrice`), **כיסוי הסניפים** (`storeCoverage`, 0-1), ו-**`storePrices`**: רשימת `"סניף:מחיר"` רק לסניפים שמחירם שונה מהנפוץ. `storePrices` מוגדר `select:false`, כך שהוא לא נטען בשאילתות חיפוש, רק לברקוד הנבחר |
...
1. הסניף מופיע ב-`storePrices` של המוצר → המחיר המפורש, מאומת.
2. אין חריגה, הסניף מופיע ב-`chain_price_coverage` והמוצר בכיסוי של 90% ומעלה → המחיר הנפוץ, מאומת ומסומן כמוסק (`branchPriceInferred`).
...
> **מכסת האחסון ב-Atlas החינמי (512MB) נמדדת בגודל הלוגי, לא הדחוס, של כל בסיסי הנתונים באשכול** (נתונים ואינדקסים, כולל `smartbasket_dev`). `storageSize` שמדווח `db.stats()` קטן בהרבה ומטעה. תקרה חייבת להישען על `dataSize + indexSize`.
>
> ההיסטוריה: גרסה קודמת שמרה שורה נפרדת לכל (סניף × מוצר) באוסף `branch_prices` עם אינדקס מורכב (כ-260 בתים לשורה). זה מילא את המכסה וחסם כתיבות בכל האשכול. חריגה בתוך המסמך עולה כ-15 בתים, בלי אינדקס.

**תקציב חריגות לרשת** (`MAX_EXCEPTION_ROWS_PER_CHAIN`, 300 אלף, כ-4.5MB): רשת שמחיריה משתנים מאוד בין סניפים ועוברת אותו **לא נשמרת ברמת סניף בכלל** (הכיסוי נמחק), ונשארת בהשוואה ברמת רשת מסומנת "לא מאומת" (ההתראה `exceed budget` בלוג הסנכרון). זו החלטה מכוונת: חלקי-נתונים היה גרוע יותר, כי היעדר חריגה מוסק כמחיר הנפוץ.
...
- **מה קורה לכל רשת** (`services/priceSync.service.ts`): הורדת קבצי המחירים ← אגרגציה לפי ברקוד (מחיר מינימלי, נפוץ, כיסוי) ← שמירה ב-`prices` ← סנכרון סניפים ← חישוב מחיר נפוץ, כיסוי וחריגות לכל ברקוד ← שמירה ב-`prices` (כולל `storePrices`, שמחליף את הקודם במלואו) ← רישום `chain_price_coverage`. רשת בלי מזהי סניף או שחרגה מהתקציב מקבלת `storePrices` ריק והכיסוי שלה נמחק.
...
- **נפח:** חריגות המחיר הן כ-15 בתים כל אחת בתוך `prices`, בלי אינדקס. לבדיקה: `db.stats()` והשוואה של `dataSize + indexSize` (לא `storageSize`) של כל בסיסי הנתונים ל-512MB. רשת ב"exceed budget" בלוג אינה תקלה, אלא רשת שחוזרת להשוואה ברמת רשת.
...
- **הרשאות:** משתמש חינמי מוגבל במספר השוואות ליום (`PLAN_LIMITS`), בכפוף לדגל `FREEMIUM_ENABLED` (ראו הערת סביבה).
