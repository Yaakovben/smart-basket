...
**תקציב שורות (רשת ביטחון):** שורת חריגה עולה כ-130 בתים (נתונים ואינדקסים), ולכל רשת מותרות עד **150 אלף** שורות (`MAX_EXCEPTION_ROWS_PER_CHAIN`, `services/branchPricing.ts`). שתי סיבות לשמור שורה, והן מטופלות אחרת:
...
- **נפח:** התקציב לרשת מחזיק את `branch_prices` על כ-275MB בשיא (14 רשתות), ועוד כ-70MB ל-`prices`. בדיקה: `db.stats()` והשוואה ל-512MB. רשת ב"exceed budget" בלוג אינה תקלה, אלא רשת שחוזרת להשוואה ברמת רשת.
